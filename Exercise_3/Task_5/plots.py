...
def plotSIRValues(b=0.01,SIM0 = (195.3, 0.052, 4.4)):
	"""
    Function that plots SIR values based on time
    """
	# parameters
	random_state = 12345
	t_0 = 0
...
def plotAll3DTrajectories(SIM0 = (195.3, 0.052, 4.4)):
	"""
    Function that plots trajectories for different b values from 0.01 to 0.03 by 0.001 increment
    """
	random_state = 12345
	t_0 = 0
	t_end = 10000
	...
	fig.tight_layout()
def plotSpecific3DTrajectories():
	"""
    Function that plots trajectories with different starting point and b=0.021 
    """
	sims = [(195.3, 0.052, 4.4),(195.7, 0.03, 3.92),(193, 0.08, 6.21)]
	random_state = 12345
	t_0 = 0
	t_end = 10000
	NT = t_end-t_0
	...
	d=0.1
	nu=1
	b=0.021 # try to set this to 0.01, 0.020, ..., 0.022, ..., 0.03
	mu0 = 10   # minimum recovery rate
	mu1 = 10.45  # maximum recovery rate
	...
	axs = []
	for i in range(3):
		axs.append(fig.add_subplot(1,3,i+1,projection="3d"))
		sol = solve_ivp(model, t_span=[time[0],time[-1]], y0=sims[i], t_eval=time, args=(mu0, mu1, beta, A, d, nu, b), method='DOP853', rtol=rtol, atol=atol)
		axs[i].plot(sol.y[0], sol.y[1], sol.y[2], 'r-');
		axs[i].scatter(sol.y[0], sol.y[1], sol.y[2], s=1, c=time, cmap='bwr');
		axs[i].set_xlabel("S")
		axs[i].set_ylabel("I")
